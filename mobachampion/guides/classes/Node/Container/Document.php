...
class Node_Container_Document extends Node_Container
{
	...
	public static function default_bbcodes()
	{
		return array(
			...
			new BBCode('matchup', function($content, $attribs)
			{
				$muDiff = 0;
				$muShaper = "";
				$muAlign = "";
				
				if (!empty($attribs['difficulty']))
					$muDiff = $attribs['difficulty'];
				if (!empty($attribs['shaper']))
					$muShaper = $attribs['shaper'];
				if (!empty($attribs['align']))
					$muAlign = 'style="margin:0 auto; margin-bottom: 8px"';
				if (strtolower($muShaper) == "king")
					$muShaper = "King of Masks";
				if (strtolower($muShaper) == "kom")
					$muShaper = "King of Masks";
				
				if (is_numeric($muDiff))
				{
					$progType = "success";
					if ($muDiff > 66)
					{
					...
					return '<div class="guidev2_matchup">
								<h5><img src="http://www.moba-champion.com/images/shapers/' . strtolower($muShaper) . '.png">' . $muShaper . '</h5>
								<div class="guidev2_matchup_progress progress progress-' . $progType . '" ' . $muAlign . '>
									<div class="bar" style="width: ' . $muDiff . '%"></div>
								</div>
								<p>' . $content . '</p>
							</div>';
				}
